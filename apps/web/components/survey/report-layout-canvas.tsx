...
export function ReportLayoutCanvas({ chartPreview, compact = false, prompts, onPromptChange }: ReportLayoutCanvasProps) {
  ...
  return (
    <div
      data-testid="report-layout-canvas"
      className={compact ? "grid min-w-0 gap-3" : "grid min-w-0 gap-3 xl:grid-cols-[minmax(0,1fr)_280px]"}
    >
      <section className="min-w-0 border border-border bg-background">
          ...
          <div className="flex gap-1">
            {(["chart", "image", "text"] as const).map((type) => {
              ...
              return (
                <Button
                  key={type}
                  data-testid={`report-layout-select-${type}`}
                  type="button"
                  size="sm"
                  variant={selected.id === type ? "default" : "outline"}
                  className="gap-1.5"
        ...
        <div className={compact ? "bg-muted/20 p-2" : "grid grid-cols-12 gap-2 bg-muted/20 p-2"}>
          {modules.map((module) => {
            if (compact && module.id !== selected.id) return null;
            ...
            return (
              <article
                ...
                style={compact ? undefined : { gridColumn: `${module.x + 1} / span ${module.w}`, gridRow: `${module.y + 1} / span ${module.h}` }}
      ...
      <aside className="border border-border bg-background">
        ...
        <div className={compact ? "grid gap-4 p-3 md:grid-cols-[minmax(0,1fr)_220px]" : "grid gap-4 p-3"}>
          <div className="grid gap-3">
            ...
            <div className="grid gap-2">
              ...
              <Textarea id={`report-layout-prompt-${selected.type}`} data-testid={`report-layout-prompt-${selected.type}`} value={selected.prompt} onChange={(event) => patchPrompt(event.target.value)} className={compact ? "min-h-20" : "min-h-28"} />
              <Button type="button" size="sm" className="gap-1.5"><RotateCcw className="h-4 w-4" />重新生成</Button>
            </div>
          </div>
          <div className={compact ? "border-t border-border pt-3 md:border-l md:border-t-0 md:pl-3 md:pt-0" : "border-t border-border pt-3"}>
            ...
            {selected.type === "chart" ? <p className="mt-3 border-t border-border pt-3 text-11 leading-5 text-muted-foreground">X 轴：题目选项 · Y 轴：回答人数 · 聚合：计数</p> : null}
          </div>
        </div>
      </aside>
    </div>
