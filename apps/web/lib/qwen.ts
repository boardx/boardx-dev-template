...
export async function callQwenJson<T>(input: {
  ...
  const messages = input.messages.some((message) => /json/i.test(message.content))
    ? input.messages
    : [
        { role: "system" as const, content: "Return one valid JSON object only." },
        ...input.messages,
      ];
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `千问请求失败 (${response.status})${detail ? `: ${detail}` : ""}`
    );
  }
