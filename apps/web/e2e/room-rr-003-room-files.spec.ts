...
test("安全回归：confirm 忽略客户端伪造的 objectKey，storage_path 恒由服务端按 roomId+fileId 重新推导", async ({
  page,
}) => {
  const owner = await register(page, "owner");
  const roomA = await createRoom(page, "RoomA");
  const roomB = await createRoom(page, "RoomB");

  // 在 roomA 走一次合法预签名拿到真实 fileId，然后向 roomB 的 confirm 提交
  // 一个伪造的 objectKey（指向别的房间/别的命名空间），验证服务端不采信它。
  const presignRes = await page.request.post(`/api/rooms/${roomB.id}/files`, {
    data: { fileName: "legit.txt", fileSize: 4, contentType: "text/plain" },
  });
  expect(presignRes.status()).toBe(200);
  const { fileId, uploadUrl } = (await presignRes.json()) as { fileId: string; uploadUrl: string };
  await page.request.put(uploadUrl, { data: "ok", headers: { "content-type": "text/plain" } });

  const forgedObjectKey = `rooms/${roomA.id}/files/${fileId}/legit.txt`;
  const confirmRes = await page.request.post(`/api/rooms/${roomB.id}/files/confirm`, {
    data: { fileId, objectKey: forgedObjectKey, fileName: "legit.txt", fileSize: 4, chatThreadId: null },
  });
  expect(confirmRes.status()).toBe(201);
  const { file } = (await confirmRes.json()) as { file: { id: string; storage_path?: string } };

  // 落库的 storage_path 必须是服务端用 roomB 重新算出来的，绝不能是客户端伪造的 roomA key。
  expect(file.storage_path).toBe(`rooms/${roomB.id}/files/${fileId}/legit.txt`);
  expect(file.storage_path).not.toBe(forgedObjectKey);

  // fileId 格式非法（含路径分隔符，尝试路径穿越）→ 400，不落库。
  const badFileIdRes = await page.request.post(`/api/rooms/${roomB.id}/files/confirm`, {
    data: { fileId: "../../../kb/other/secret", objectKey: "irrelevant", fileName: "x.txt", fileSize: 1 },
  });
  expect(badFileIdRes.status()).toBe(400);
});

test("安全回归：confirm 里 chatThreadId 必须真的属于当前房间，否则 400", async ({ page }) => {
  const owner = await register(page, "owner");
  const roomA = await createRoom(page, "ChatRoomA");
  const roomB = await createRoom(page, "ChatRoomB");

  const chatInB = (await (await page.request.post(`/api/rooms/${roomB.id}/chats`, { data: { name: "ChatB" } })).json())
    .chat;

  const presignRes = await page.request.post(`/api/rooms/${roomA.id}/files`, {
    data: { fileName: "cross-room.txt", fileSize: 4, contentType: "text/plain" },
  });
  const { fileId, uploadUrl } = (await presignRes.json()) as { fileId: string; uploadUrl: string };
  await page.request.put(uploadUrl, { data: "ok", headers: { "content-type": "text/plain" } });

  // 把 roomA 的文件 confirm 时标注成属于 roomB 的线程 → 拒绝。
  const confirmRes = await page.request.post(`/api/rooms/${roomA.id}/files/confirm`, {
    data: { fileId, objectKey: "irrelevant", fileName: "cross-room.txt", fileSize: 4, chatThreadId: chatInB.id },
  });
