...
test("Deep Research：提交主题 → 两步确认（澄清→计划）→ 执行时间线 → 报告面板 → 继续追问", async ({
  page,
}) => {
  await register(page);
  await page.goto("/ava");
  ...
  // p18-F04：内容由真实生成引擎针对 topic 产出（stub provider 下也随 topic 变化），
  // 不再断言固定文案——只断言澄清问题里出现了 topic 本身的关键词。
  ...
  await expect(page.getByTestId("research-clarify")).toContainText("collaborative");
  await expect(page.getByTestId("research-plan")).toContainText("Audience:");

  // 两步确认：先确认澄清问题，此时计划确认按钮还不存在（不能跳过澄清直接确认计划）。
  ...
  await expect(page.getByTestId("report-conclusion")).toContainText("collaborative");
