...
test("底部工具 dock 可见可用，Ask AI 唤起 board chat 面板并可就画布内容问答", async ({ page }) => {
  ...
  await expect(panel.getByTestId("board-ai-msg-ai")).toContainText("总结");

  // 再追问一次，验证消息持续累积（真实对话，而非一次性 mock）
