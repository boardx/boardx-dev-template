...
import { closePool, getKbFile, setKbFileStatus } from "@repo/data";
...
test("处理中（未 ready）的文件不参与检索，不出现在引用里", async ({ page }) => {
  ...
  // 上传后默认 processing，但环境里若有 workflow-worker 在跑（verify-full/CI e2e-full
  // 都会起），它会异步把文件消费成 ready，造成竞态。先等 worker 消费完（无 worker 则
  // 超时后本来就是 processing），再强制拨回 processing，保证查询时刻状态确定。
  await expect
    .poll(async () => (await getKbFile(processing.id))?.status, { timeout: 20_000 })
    .not.toBe("processing")
    .catch(() => {
      /* 无 worker 环境：一直是 processing，正是本用例要的状态 */
    });
  await setKbFileStatus(processing.id, "processing");
