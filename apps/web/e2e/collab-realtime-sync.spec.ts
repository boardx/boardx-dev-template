...
test("一个便签编辑中不阻塞另一个便签的实时同步", async ({ browser }) => {
  ...
  try {
    ...
    await pageA.getByTestId("add-note").click();
    await expect(itemLocator(pageA)).toHaveCount(1);
    const editingId = (await itemLocator(pageA).first().getAttribute("data-testid"))?.replace("item-", "");
    await pageA.getByTestId("add-note").click();
    await expect(itemLocator(pageA)).toHaveCount(2);
    await expect(itemLocator(pageB)).toHaveCount(2, { timeout: 5_000 });
    const movingId = (await itemLocator(pageA).nth(1).getAttribute("data-testid"))?.replace("item-", "");

    // 用 id 精确定位，不依赖 DOM 顺序（两个页面各自维护的 items 数组顺序不保证一致）。
    const editingNoteA = pageA.getByTestId(`item-${editingId}`);
    const movingNoteA = pageA.getByTestId(`item-${movingId}`);
    const movingNoteB = pageB.getByTestId(`item-${movingId}`);

    // A 进入第一个便签的编辑态，先不提交。
    await editingNoteA.dblclick();
    await pageA.getByTestId(/^item-edit-/).fill("A 正在打字，还没提交");

    // B 移动第二个便签：与 A 正在编辑的不是同一条，应该不受 A 编辑态影响，很快同步给 A。
    const before = await movingNoteA.evaluate((el) => (el as HTMLElement).style.left);
    await movingNoteB.click();
    await pageB.keyboard.press("ArrowRight");
    await expect
      .poll(async () => (await movingNoteA.evaluate((el) => (el as HTMLElement).style.left)), { timeout: 1_200 })
      .not.toBe(before);
