...
async function submitTeamItemForReview(page: import("@playwright/test").Page, name: string) {
  ...
  await expect(page.getByTestId("saved")).toContainText("已提交审核");
...
test("团队 owner 审核团队项目：提交审核后出现在待审队列，通过确认弹窗批准后发布并移出队列", async ({ page }) => {
  ...
  expect(
    browseData.items.some(
      (it: { id: number | string; status: string }) => Number(it.id) === itemId && it.status === "published"
    )
  ).toBeTruthy();
});
...
test("团队 owner 拒绝审核项目：拒绝后移出待审队列，不进入精选", async ({ page }) => {
  ...
  // 服务端确认状态为 rejected（直接调用团队审核动作应因状态不合法而 404）。
...
test("团队 owner 切换精选状态，UI 立即反映；精选只对已批准项目生效", async ({ page }) => {
  ...
  // 精选只能作用于已批准（published）项目：对一个仍在 PENDING 的项目直接调用精选接口应 404。
  ...
  expect(featureRejectRes.status()).toBe(404);
});
