...
test("混选：可刷新 + 不可刷新一起选 → 隐藏刷新入口（退化为不可用）", async ({ page }) => {
  ...
  // addEmbed 先以 note 落库再异步 PATCH 写入 color 哨兵（两步异步，同 addText/addShape
  // 已知的创建时序），必须等 reloadable 落定再点 add-note，否则第二个 setSelected([noteId])
  // 可能与 addEmbed 里稍后才 resolve 的 setSelected([embedId]) 交叉，选中态互相覆盖。
  await expect.poll(async () => (await canvasItems(page)).some((it) => it.reloadable)).toBe(true);
