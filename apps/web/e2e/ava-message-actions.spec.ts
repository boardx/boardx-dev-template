...
test("消息操作条：复制整条消息、代码块单独复制、点赞/点踩持久化、重新生成不丢原问题", async ({
  ...
  // ── 发送到 Board / 发送邮件：禁用占位，不可点击 ──────────────────
  await expect(page.getByTestId("msg-send-to-board").last()).toBeDisabled();
  await expect(page.getByTestId("msg-send-email").last()).toBeDisabled();
