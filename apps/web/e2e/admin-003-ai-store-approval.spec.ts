...
test("revoke 作用在从未被 approve 过的 pending 项目上，必须返回 409（不能谎报 idempotent:true）", async ({
  page,
}) => {
  // 回归用例：修复 PR #213 code review 发现的幂等回退 bug——revoke 的目标状态 pending
  // 同时也是 approve/reject 的天然前置状态，一个从未被 approve 过、本来就是 pending 的
  // item 直接调 revoke，UPDATE 命中 0 行后，若只用"当前状态==目标状态"判定幂等，会误判为
  // "revoke 已经生效过"而返回 200 idempotent:true——但这其实是非法转移（item 从没被
  // approved 过），必须是 409。
  await registerAndPromote(page);
  const itemName = `Review Revoke Never Approved ${Date.now()}`;
  ...
  const create = await page.request.post("/api/ai-store/items", {
    data: {
      ...
      action: "submit_review",
      name: itemName,
      description: "revoke on never-approved pending item must 409",
      config: "instructions",
      tags: "research",
    },
  });
  expect(create.status()).toBe(201);
  const item = (await create.json()).item as { id: number; status: string };
  expect(item.status).toBe("pending");

  const res = await page.request.post(`/api/admin/ai-store/${item.id}/review`, { data: { action: "revoke" } });
  expect(res.status()).toBe(409);

  // 越权侧确认：item 状态确实没被悄悄改动——仍在 pending 审核队列里可查到，不是被误判幂等后
  // "顺手"落地到别的状态（比如悄悄变成 approved 却对外报 409，那也是数据损坏）。
  const listRes = await page.request.get(`/api/admin/ai-store?status=pending&q=${encodeURIComponent(itemName)}`);
  expect(listRes.status()).toBe(200);
  const listBody = await listRes.json();
  expect(listBody.items.some((it: { id: number; status: string }) => it.id === item.id && it.status === "pending")).toBe(
    true,
  );
});
