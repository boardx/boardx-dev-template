...
test("应用格式：设置源便签样式 → 进入取样模式 → 点击目标便签 → 样式复制过去", async ({ page }) => {
  ...
  // applyFormatTo 的 PATCH 经 queuePatch 异步落库，与本文件其它用例一致（见上方 49-59 行/
  // 76-78 行同类断言）：client 侧 canvasItems 是乐观更新，会先于服务端落库变为目标值，
  // 故服务端断言须用 expect.poll 等落库，不能一次性 GET（真实回归：偶发因 PATCH 尚在途中
  // 读到旧值 null，而非应用格式逻辑本身有误——见诊断记录）。
  const targetColor = () =>
    page.request
      .get(`/api/boards/${board.id}/items`)
      .then((r) => r.json())
      .then((j) => j.items.find((it: { id: string }) => it.id === targetId).color as string);
  await expect.poll(targetColor).toContain("blue"); // 背景色也随格式一并复制
  await expect.poll(targetColor).toContain(":bold");
  await expect.poll(targetColor).toContain("border=red");
  await expect.poll(targetColor).toContain("opacity=75");
