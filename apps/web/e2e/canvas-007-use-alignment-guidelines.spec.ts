...
const itemsLoc = (page: any) => page.getByTestId("items-layer").locator('[data-testid^="item-"]');
...
test("拖动组件靠近另一组件边缘 → 出现对齐参考线并吸附对齐", async ({ page }) => {
  ...
  await page.getByTestId("add-note").click();
  await expect(itemsLoc(page)).toHaveCount(2);
  ...
  const box1 = await page.getByTestId(`item-${note1.id}`).boundingBox();
  ...
  const box1b = await page.getByTestId(`item-${note1.id}`).boundingBox();
