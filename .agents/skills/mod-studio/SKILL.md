...
name: mod-studio
description: >
  激活条件：接到 Studio / Presentations（生成式演示） 模块的 feature/bug/review/勘探任务时，动手前先读本 skill。
  这是该模块的活知识库（定位/代码地图/契约/经验），由每个在此干活的开发者与 agent
  持续回流迭代（规则见文末）。
---

# Studio / Presentations（生成式演示） — 模块知识库

> 本文件是 studio 模块的**单一经验沉淀点**（人类拍板 2026-07-12：每模块一个 skill，
> 让任何开源开发者都能持续迭代模块的 SOP/技巧/知识结构）。读完你应该知道：
> 代码在哪、什么不能破坏、前人踩过什么坑。

## 一句话定位
从 board/对话生成 artifact 与演示文稿：生成、渲染、导出。

## 代码地图
- 页面：`apps/web/app/(app)/studio/`、`apps/web/app/(app)/presentations/`
- 组件：`apps/web/components/studio/`、`components/presentations/`

## 关键契约与不变量（改代码前必读）
- 生成类操作必须有进行中/失败/重试三态，不允许静默失败。
- Studio 与 Room 的归属边界（独立性 vs 面包屑回退）见 p20 需求评估。

## 关联阶段 / ADR / 文档
phases/phase-p12-studio-presentations
...
- （待第一条回流；p12 的 e2e evidence 是老格式 .txt，doctor 会 WARN——补跑时顺带规范化）
