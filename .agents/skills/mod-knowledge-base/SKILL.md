---
name: mod-knowledge-base
description: >
  激活条件：接到 Knowledge Base（知识库） 模块的 feature/bug/review/勘探任务时，动手前先读本 skill。
  这是该模块的活知识库（定位/代码地图/契约/经验），由每个在此干活的开发者与 agent
  持续回流迭代（规则见文末）。
---

# Knowledge Base（知识库） — 模块知识库

> 本文件是 knowledge-base 模块的**单一经验沉淀点**（人类拍板 2026-07-12：每模块一个 skill，
> 让任何开源开发者都能持续迭代模块的 SOP/技巧/知识结构）。读完你应该知道：
> 代码在哪、什么不能破坏、前人踩过什么坑。

## 一句话定位
个人/Team 文件知识库：上传/列表/下载/删除 + 在 AI 上下文中引用（CAP-FILE + AI）。

## 代码地图
- 页面：`apps/web/app/(app)/knowledge-base/`
- 存储：`packages/storage`（S3 兼容层，ensureBucket 自动建桶）
- AI 引用侧：packages/ai 上下文注入

## 关键契约与不变量（改代码前必读）
- 一切对象存储操作走 packages/storage，S3_* env 可切 MinIO/R2/真 S3（部署可移植性约束）。
- 上传大小/类型校验在服务端，客户端校验只是体验优化。

## 关联阶段 / ADR / 文档
phases/phase-p10-knowledge-base
