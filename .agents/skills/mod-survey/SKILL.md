...
- 2026-07-31：报告源快照同时受 `source_revision` 主键和 `(survey_id, content_hash)` 唯一键约束；
  并发加载报告上下文时，幂等写入必须对任一唯一冲突执行 `DO NOTHING`，再按两个稳定键回读已有快照，
  不能只指定其中一个冲突目标，否则另一个唯一键仍会抛错（出处：phase-p25 F25）。
