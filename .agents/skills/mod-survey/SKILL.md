...
- 2026-07-20：持久工作流壳层还必须统一拥有满宽内容框、页面边距和标题层级；只保持步骤导航 DOM
  稳定仍会因各步骤自行选择 `max-width` 和重复命令栏而形成视觉割裂。用同一内容容器约束五步，
  仅允许步骤内部按任务选择单列或多栏结构（出处：phase-p25 F25 / issue #805）。
- 2026-07-19：五步 Survey 工作流不能由设计器和各工作台分别渲染页面头部/步骤条；公共返回区、
  问卷标识和步骤导航应由单一持久壳层拥有，步骤切换只替换导航下方内容，并用浏览器断言验证
  DOM 节点身份和几何位置不变（出处：phase-p25 F24 / issue #648）。
- 2026-07-19：专业报告必须在领取生成 claim 和调用模型前校验有效答卷数；零答卷应返回明确业务错误，
  页面禁用生成入口并引导先回收答卷，不能把数据前置条件包装成通用 500 或暴露内部错误码
  （出处：phase-p25 F23 / issue #648）。
- 2026-07-19：专业报告阅读页不能按历史产物与模板驱动产物分叉成不同 UI；两者应统一为单列阅读面，
  章节编辑和 AI 修订只留在报告模板步骤，并用入口 E2E 断言不会再请求旧 `/ai-report`
  （出处：phase-p25 F22 / issue #648）。
...
- 2026-07-20：工作流 Tab 离开设计页时必须使尚未完成的编辑器加载请求失效；否则迟到的
  `setMode("editor")` 会覆盖用户刚选择的模板或报告步骤。零答卷报告读取应直接从已保存模板快照构建
  同顺序只读框架，禁止调用模型或填充模拟图表、图片和结论（出处：phase-p25 F26 / issue #811）。
