...
2. lease 机制同 module-coordinator（`coordination:lease:architecture`），但你巡检的
   不是 issue 队列，而是"文档 vs 实际协作事故"的落差——定期读总协调 lease issue
   （`coordination:lease`，通常是 #323）和各模块 coordinator 的报到评论，收集
