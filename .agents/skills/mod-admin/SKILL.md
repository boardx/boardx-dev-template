...
name: mod-admin
description: >
  激活条件：接到 Admin（管理后台） 模块的 feature/bug/review/勘探任务时，动手前先读本 skill。
...
> 本文件是 admin 模块的**单一经验沉淀点**（人类拍板 2026-07-12：每模块一个 skill，
> 让任何开源开发者都能持续迭代模块的 SOP/技巧/知识结构）。读完你应该知道：
> 代码在哪、什么不能破坏、前人踩过什么坑。

## 一句话定位
SysAdmin 面板：用户/协调状态（admin/coordination）等内部运维界面——敏感 area。

## 代码地图
- 页面：`apps/web/app/(app)/admin/`
- 协调代理：`apps/web/app/api/admin/coordination/`（读 coord-gateway RepoHub claims/events，SysAdmin 门禁；ADR-017 割接）

## 关键契约与不变量（改代码前必读）
- **admin 是敏感 area**：强制 rev-security。
- 门禁基线 requireSysAdmin；比 portal（登录即可）严——两者门禁不同是有意设计，别"顺手统一"。

## 关联阶段 / ADR / 文档
phases/phase-p15-admin
...
- （待第一条回流；p15 有 doctor FAIL 存量待还，见 ADR-012 D4）
