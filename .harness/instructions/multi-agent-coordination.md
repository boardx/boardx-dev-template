...
| `coordination:lease` | 总 coordinator（coord-main）唯一性心跳锁 | coordinator 生命周期仪式（见 coordinator-sop.md） |
| `coordination:lease:<module>` | 该 module-coordinator 的唯一性心跳锁（`<module>` = registry.yaml 中 kind 为 `module-coordinator`/`architecture-coordinator` 的 id 去掉 `coord-` 前缀，如 `room`/`board`/`architecture`） | module-coordinator/architecture-coordinator 启动仪式（见对应 SKILL.md） |
