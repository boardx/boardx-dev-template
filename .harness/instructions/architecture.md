...
### 生成式结构化界面：A2UI（ADR-021，2026-07-21 起）

LangGraph 节点产出"下一步说什么/做什么"，但当产出本身是**一棵用户要交互式编辑
的结构化界面**（不是一段文字）——如报告大纲的章节可重排、可编辑——纯文本/
markdown 装不下这种交互，手搓渲染器又是在重新发明"预批准组件目录 + agent 填
数据"这套安全模型。这类场景用 **A2UI**（声明式生成式 UI 协议，Google）经
**CopilotKit 的 AG-UI**（LangGraph 官方支持的传输层）承载：

```
LangGraph 节点（编排，ADR-020）→ CopilotKit AG-UI（传输，事件流+状态同步）
  → A2UI（声明式 UI 载荷：组件+数据的 JSON 树）→ React 渲染器（apps/web）
```

**判断信号**：agent 需要在运行时决定一棵结构、且用户需要交互式编辑这棵结构，
才值得上这一层；单纯"生成一段话/一份只读报告"不需要，继续用 LangGraph 直出
文本或结构化 JSON 交给 hand-built 组件渲染即可（同上文 Deep Research 的模式，
不必每个功能都套生成式 UI 协议）。首个落点：Survey Dynamic Report Engine 的
Report Composer 输出、Survey Report Agent 的可编辑大纲预览，完整评估见 ADR-021。
...
- `docs/adr/ADR-021-a2ui-generative-report-surface.md` — 生成式结构化界面用 A2UI 经 CopilotKit AG-UI 承载。
