...
每个在任 coordinator（coord-main + 各 module-coordinator）在自己的 lease issue 发一条
`cycle-plan` 评论，格式固定（机器可解析）：
```
...
| `in_progress` 无任何可见进展 | 1 个周期 | 通牒（15-30 分钟窗口）→ 回收。替代现在的 6h lease TTL |
...
2. **`pnpm harness cycle-report`（可选但值得）**：读各 lease issue 最近的 cycle-plan/result
   评论 + open PR 的 createdAt/updatedAt，输出一张"当前周期健康表"（谁承诺了什么/什么超 SLA/
   flow time 趋势）。dry-run 只读，无 --apply 概念。可以挂进 coordination dashboard
   （#428 已上线第一片）作为后续卡片。
