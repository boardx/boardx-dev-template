...
> status: Accepted（PR #443 合并 + 2026-07-08 amendments：与 ADR-009 对齐的三处修正，
> 由 architecture-coordinator review 提出、coord-main 全盘接受，见 PR #443 评论）
...
`[coordination] work-cycle` issue**（全仓唯一，label `coordination:work-cycle`；
amendments 修正——lease issue 已随 ADR-009 退役，且聚合在单一 issue 里比散在各处
更好汇总）发一条 `cycle-plan` 评论，格式固定（机器可解析）：
```
...
| `in_progress` 无任何可见进展 | 1 个周期 | worker 对 `issue:<n>` 做 D1 claim 时传 `ttl_seconds=10800`（3h），由服务端 sweeper 机械过期回收（amendments 修正：机械执行替代人肉通牒巡检——通牒仅用于有争议场景）|
...
- **不做新的常驻服务**——cycle-plan/result 就是 lease issue 上的评论，复用现有总线。
...
2. **`pnpm harness cycle-report`（可选但值得）**：读单一 work-cycle issue 的
   cycle-plan/result 评论 + `gh pr list` 的 createdAt/mergedAt + coord-service
   `GET /status` 的 active_claims（谁持有什么、心跳年龄——SLA 表"in_progress 无进展"
   检测的权威数据源，amendments 修正），输出一张"当前周期健康表"（谁承诺了什么/
   什么超 SLA/flow time 趋势）。只读，无 --apply 概念。可以挂进 coordination
   dashboard 作为后续卡片。
