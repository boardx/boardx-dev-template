...
- 当前最高优先级: F07 USER and TEAM subscriptions and use
- 计划规模: 14 Features / 7 Sprints / F01-F06 `passing`，F07-F14 `not_started`
- Runtime 实现: Team 与 BoardX 审核/精选、approved 实时更新已完成；F07-F14 尚未开始。
...
- F06 evidence 位于 `sprints/sprint-03/evidence/F06.verify.log`。
- F07-F14 不存在 passing 声明或成功 evidence。
- 远程 GitHub `--apply` 未执行；当前环境没有可用的 `gh` CLI/认证。

## 下一步

1. 认领 F07: `pnpm harness claim --phase p27 --feature F07 --owner <agent-id>`。
2. 先写 `apps/web/e2e/ai-store-010-user-team-subscriptions.spec.ts` 的失败测试。
3. 完成 USER/TEAM 订阅、角色权限、Team 隔离和三类资源使用。
4. 运行 `pnpm harness verify --sprint p27/04 --feature F07`。
