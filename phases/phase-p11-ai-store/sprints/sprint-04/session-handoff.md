...
- F03「订阅并使用项目」实现已完成并本地验证通过（不是 passing——状态转移需协调者跑 `pnpm harness verify`）：
  - `pnpm --filter @repo/web exec playwright test e2e/ai-store-003-subscribe-use-item.spec.ts` → 5/5 passed（含 review 修复轮新增的 3 条回归用例）
  - F01/F02/F04/F05 回归 e2e 全绿（20/20 passed）
  - `pnpm -w run verify:base` → 45/45 successful, exit 0
- PR #214 双评审后的两个中危问题（template Use 无提示、订阅端点可见性口径不一致）已修复，见下方「review 修复轮改动」。
...
## review 修复轮改动
- `apps/web/app/(app)/boards/page.tsx`：新增 `?template=` query 处理，展示
  `data-testid="template-use-notice"` 提示条，避免用户误以为已按模板建好板。
- `apps/web/app/api/ai-store/items/[id]/subscribe/route.ts`：可见性校验从 `isAiStoreItemVisible`
  换成 `canAccessAiStoreItem`，和详情路由口径统一，修复分享授权 grantee 订阅 404 的问题。
- `packages/data/src/aiStoreSubscriptions.ts`：`unsubscribeAiStoreItem` 改成先用
  `getAiStoreSubscription` 查出真实命中的行再按 id 删除，修复跨团队上下文取消订阅误报 404 的口径不对称问题。
- `apps/web/e2e/ai-store-003-subscribe-use-item.spec.ts`：新增 3 条回归用例（grantee 订阅、
  template Use 提示、跨团队取消订阅）。
