...
### Next dev 必须编译 AVA AI 包

普通本地 dev server 启动后，`/api/ava/capabilities` 必须读取当前分支的 `@repo/ai` 源码，而不是旧缓存或未编译 workspace 包导致的 stub 能力面。

可观察行为：
- `apps/web` 的 Next 配置会转译 `@repo/ai`。
- 本地 3003 重启后，已登录请求 `/api/ava/capabilities` 返回 `defaults.modelId: "qwen3.7-max"`，模型列表包含 `Qwen 3.7 Max`。
- AVA 首屏和点击 `New chat` 后的模型选择器不再因为运行时读到旧 `@repo/ai` 而显示 `Stub Default`。
