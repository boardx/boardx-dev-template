...
CAP-AI"——即当时刻意把 AI 摘要排除在核心交付之外，留到后续。现在 `packages/ai`
（`gateway.ts` 的 provider 路由 + `anthropicProvider.ts` 真实 provider + stub provider 的
确定性 e2e 模式）已经在 p18 AVA 落地中跑通（如 `researchGenerator.ts` 用同一套网关做
Deep Research 真实生成），具备复用条件，不需要再造一遍 AI 接入地基。
...
- e2e 验证走 stub provider 确定性模式（参考 `researchGenerator.ts` 的做法），不依赖真实
