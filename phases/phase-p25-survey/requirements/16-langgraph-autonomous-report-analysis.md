...
- 问卷定义和全部授权答卷共同组成同一个报告事实快照；章节不得保存或依赖用户手工选择的
  `questionIds` 作为唯一数据边界。
- 每个章节只接收章节目标、自然语言约束和同一个只读 `/source` 命名空间。分析模块根据目标
  自主检索跨题、跨答卷证据，并可明确记录“未找到足够证据”。
- 图片、图表和文本只是 F13 的章节呈现方式，三种输出必须消费同一份已验证章节结果，不得各自
  构建互相不一致的数据切片。
- 章节计划必须使用面向研究决策的专业中文标题。内部分类键如 `behavior`、`preference`
  只能作为检索元数据，不能直接成为用户可见章节标题。
- 章节结果至少包含 `conclusion`、`evidenceRefs`、`limitations` 和 `recommendation`；
  证据引用校验失败时，该章节不能进入成功报告。
