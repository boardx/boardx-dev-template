# Sprint p25/01 — 建立 Survey 全功能的数据模型、权限边界与生命周期接口

- **所属阶段**: Phase p25 (survey)
- **创建于**: 2026-07-14 07:19:31

## 本 sprint 目标
建立 Survey 全功能的数据模型、权限边界与生命周期接口

## 领取的 feature(引用自阶段权威清单,按 id)
- F01 (P1, survey) — Survey 数据模型、权限与生命周期接口
...
## 完成标准
- 上述每个 feature 经 `pnpm harness verify --sprint p25/01` 门控为 `passing`。
