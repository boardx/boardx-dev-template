...
## 4. 协调层接入门禁（2026-07-18 更新：coord-service 已退役，走 coord-gateway）

**历史注记**：本节原文写的是"coord-service 暂不开放给新 agent"（3 个 fail-open
修复 + soak 门槛）。该服务未再开放，问题按 ADR-017 以**重建**收场：2026-07-18 起
协调权威 = coord-gateway（按仓 RepoHub DO，DO 单线程从结构上消灭了那 3 类
fail-open），`packages/coord-service` 已删除，`COORD_SERVICE_URL`/
`COORD_SERVICE_TOKEN` 永久退役。**新 agent 接入协调层**：按 `agent-bootstrap.md`
第 3 步领按仓 scoped token（devportal 自助领取，p29-F08），配置
`COORD_GATEWAY_URL`/`COORD_API_TOKEN`/`COORD_REPO` 即可 `lock-*`/`module-lock-*`/
`tick`——不再有"等修复落地"的门禁。
...
- reviewer 产能是否也要扩（目前只有 4 个 reviewer，`rev-code`/`rev-feature` 的
  ...
  提案范围，需要用户明确是否要一起扩。
- ~~coord-service 的 3 个修复何时落地、soak 何时完成~~（已了结：ADR-017 重建取代
  修复，coord-gateway 已上线并完成 F10 割接，2026-07-18）。
